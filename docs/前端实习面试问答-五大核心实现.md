...
## 五、核心实现 ③：资料 Markdown 分块与驱动出题

### 5.1 我做了什么

1. **分块**：`material-chunk-splitter.ts`
  - 按 `#` ~ `####` 标题层级切段；  
  - 单块 ≤ **1000** 字符，过长按窗口切分（带 100 字符重叠策略防断句）；  
  - 输出 `MaterialChunk`：`heading`、`level`、`text`、`charStart`/`charEnd`。
2. **出题**：`material-question-builder.ts` 规则引擎：章节讲解题、代码块题、面试题样式标题等，写入资料题池。
3. **组卷/模拟**：`PersistedPracticeQuestionGroup` 携带 `sourceChunkId`、`sourceDocumentId`，模拟面试请求里带 `sourceDocumentExcerpt` 等字段。
...
- 用户导入 `.md` 资料后，可按**章节**浏览块、生成题池，不必整篇塞进 prompt。
- 单块上限 1000 字，控制后续 LLM token 与前端列表渲染成本。
- `charStart`/`charEnd` 支持报告页回溯「答案来自哪一段资料」。
...
**Q4：为什么不用 PDF.js / 服务端解析就好？**

**答：**

- 前端分块：资料已在 `rawText`，本地即时、无额外接口；与「展示层 + 题池」边界一致。  
- DOCX 等可后续走解析服务，但**分块算法**仍可复用，输入统一成 Markdown 文本。
